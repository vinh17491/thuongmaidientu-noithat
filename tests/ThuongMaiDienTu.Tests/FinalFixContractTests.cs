...
public sealed class FinalFixContractTests
{
    ...
    public void EveryHouseholdSeedImageExists()
    {
        ...
        foreach (var name in names)
            Assert.True(File.Exists(Path.Combine(Root, "src", "ThuongMaiDienTu", "wwwroot", "images", "products", name + ".svg")), name);
    }

    private static string Read(params string[] parts)
    {
        var path = parts[0] == "ThuongMaiDienTu"
            ? Path.Combine([Root, "src", .. parts])
            : Path.Combine([Root, "database", .. parts]);
    ...
    private static string FindRoot()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "database", "thuongmaidientu.sql")))
